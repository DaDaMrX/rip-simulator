...
class RIP:
    ...
    def __init__(self, port, neighbors, verbose):
        ...
        print(f'RIP Bind on {self.port}, Neighbors: {list(self.neighbors)}')
    ...
    def parse_commands(self, commands):
        ...
        elif commands[0] == 'P':  # Specified priority route*
            num = int(commands[1])
            ports = list(map(int, commands[2:]))
            self.priority = ports
            print(f'[Priority] Set priority as {ports}')
        else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='RIP Simulative Process')
    parser.add_argument('port', type=int)
    parser.add_argument('neighbor_ports', type=int, nargs='+')
    parser.add_argument('--verbose', '-v', action='store_true')
    args = parser.parse_args()
    rip = RIP(args.port, args.neighbor_ports, args.verbose)
    rip.start()
